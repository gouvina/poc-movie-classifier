...
from utils.const import IMAGES_ROUTE, ResizeTypes, ThresholdTypes
import processing.ocr as ocr

def get_images():
    '''Get list of image names'''
    return listdir(IMAGES_ROUTE)

def post_image(image_name, image_url, resize_type, threshold_type):
    '''Get image from url and save it locally'''
    try:
        urlretrieve(image_url, IMAGES_ROUTE + image_name)
...
def get_image_by_name(name, resize_type, threshold_type):
    ...
    images = [p for p in listdir(IMAGES_ROUTE) if p.split('.')[0] == name]
    if not images:
        return {}

    image_text = ocr.get_image_text(IMAGES_ROUTE + images[0])
    result = {
        'name': images[0],
        'text': image_text,
    }
    ...
    if resize_type == ResizeTypes.ENLARGE:
        resized_image_text = ocr.get_enlarged_image_text(IMAGES_ROUTE + images[0])
        result['resized'] = {
            'enlarged': resized_image_text
        }
    elif resize_type == ResizeTypes.REDUCE:
        resized_image_text = ocr.get_reduced_image_text(IMAGES_ROUTE + images[0])
        result['text_resized'] = {
            'reduced': resized_image_text
        }
    elif resize_type == ResizeTypes.BOTH:
        resized_image_text = ocr.get_resized_image_text(IMAGES_ROUTE + images[0])
        result['text_resized'] = {
    ...
    if threshold_type == ThresholdTypes.ALL:
        threshold_image_text = ocr.get_threshold_image_text(IMAGES_ROUTE + images[0], resize_type)
